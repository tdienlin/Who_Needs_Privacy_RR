...
d_raw <- sjlabelled::read_spss(
  "data/Need+for+Privacy_February+6%2C+2024_15.56.sav",
...
vars_pers_fac_txt <- c("Sincerity", "Fairness", "Greed avoidance", "Modesty", 
                       ...
                       "Aesthetic\nappreciation", "Inquisitiveness", "Creativeness", "Unconventionality")
...
First filter those who didn't finish the questionnaire.
...
Only 23 cells are missing -- which is very little. To be precise, 0.00006 percent only. 
...
However, this made little sense. The median response time was `r time_med` and the standard deviation `r time_sd`. Hence, three SDs below median was `r time_crit_init`, hence not informative. 
...
Let's inspect for potential response patterns.
...
Let's first inspect age. Here, the age categories were as follows:
...
Let's next inspect gender. Categories were: 
...
Let's next inspect education. Categories were: 
...
Let's next inspect ethnicity. Answer categories were as follows:
...
No item below floor (1.5) or ceiling (6.5) thresholds.
...
Let's first look at relations between need for privacy and personality factors.
...
Doesn't converge. Hence let's use means of observed values for personality and for privacy. Let's specify errors as preregistered.
...
```
Throws a warning: "covariance matrix of latent variables is not positive definite." Let's hence look at correlations of observed mean scores. Also do in subsequent analyses (i.e., facets and SES), to maintain comparability.
...
```{r cache=F, fig.height=6, fig.width=12}
# First extract data
tab_cor <- 
  d %>% 
  correlation(
